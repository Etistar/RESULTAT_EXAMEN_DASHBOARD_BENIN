...
```{r}
#La libraire raster permet de télécharger des cartes avec des subdivisions: Département, communes
```
...
```{r }
resultat_examen <- read_excel("C:/Users/DPCS6481/Documents/examen/resultat_examen.xlsx")
...
m1 = leaflet(carte) %>% addProviderTiles("CartoDB.Positron",option = providerTileOptions(opacity =0.99)) %>% addPolygons(
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto")
  ) %>% leaflet::addLegend(pal = pal, values = ~carte$CEP, opacity = 0.7, title = NULL,
...
m2 = leaflet(carte) %>% addProviderTiles("CartoDB.Positron",option = providerTileOptions(opacity =0.99)) %>% addPolygons(
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto")
  ) %>% leaflet::addLegend(pal = pal, values = ~carte$BEPC, opacity = 0.7, title = NULL,
  position = "bottomright")
...
m3 = leaflet(carte) %>% addProviderTiles("CartoDB.Positron",option = providerTileOptions(opacity =0.99)) %>% addPolygons(
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto")
  ) %>% leaflet::addLegend(pal = pal, values = ~carte$BAC, opacity = 0.7, title = NULL,
  position = "bottomright")
...
Evolution <- read_delim("C:/Users/DPCS6481/Documents/examen/Evolution.csv",  ";", escape_double = FALSE, col_types = cols(Annee = col_date(format = "%d/%m/%Y")), trim_ws = TRUE)


don=xts::as.xts( x=Evolution[,2:3], order.by=Evolution$Annee)

#don = data.frame(don)

a = dygraph(don) %>%
  dySeries("BAC", label = "BAC") %>%
  dySeries("BEPC", label = "BEPC") %>%
...
labels = paste("<strong> Localisation : </strong>", loc , "<br>",
               "<strong> Rang : </strong>", carte$rang_moyen , "<br>",
...
m2 = leaflet(carte) %>% addProviderTiles("CartoDB.Positron",option = providerTileOptions(opacity =0.99)) %>% addPolygons(
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto")
  ) %>% leaflet::addLegend(pal = pal, values = ~carte$taux_moyen, opacity = 0.7, title = NULL,
  position = "bottomright")
